...
do i=0,meshN-1
   ...
   !F_an(i) = S*(exp(sqr*x(i)) - exp(sqr*(2*meshN - x(i))))/((1-exp(2*meshN*sqr)))
    !F_an(i) = (-S/D)*cosh(sqr*x(i))
    !F_an(i) = S*((-(S_mult-exp(sqr*meshN))/(2*sinh(sqr*meshN))*exp(-sqr*meshN) &
    !+ (1+(S_mult-exp(sqr*meshN))/(2*sinh(sqr*meshN)))*exp(sqr*meshN)))
end do
...
mat_B(1) = S
!mat_B(1) = F(1)
mat_B(meshN - 2) = F(meshN-1)
!mat_B(meshN - 1) = F(meshN-1)
print *,'B:', mat_B


!print *,'A Array OUTSIDE:', mat_A
print *, 'SHAPE: ', SHAPE(mat_A)
A_inv = inv(mat_A, mat_b)
...
!  ! LU factorization computed by DGETRF.
  ...
  !if (info /= 0) stop 'Solution of the linear system failed!'
   ...
   call DGETRS('N', n, 1, Ainv, n, ipiv, b, n, info)  
   if (info /= 0) stop 'Solution of the linear system failed!'
