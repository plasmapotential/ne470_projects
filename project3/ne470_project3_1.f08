...
real(real64), ALLOCATABLE :: fis_mat(:,:)
...
ALLOCATE(fis_mat(init_a, init_a))
...
!   !Collapse Fission Matrix across groups (3rd dim.) for S calculation
!   fis_mat=0
!   fis_mat = sum(sigma_mat, DIM = 3)

   !Initial Guesses
   flux = 1
   k = 1
   !Build Source Matrix
   S = matmul(sigma_mat(:,:,1), flux(:,1))
   RHS = 1/k*X(1)*S
   test = 1
   ...
   do while(test >= tol1)
      ...
      do g=1,gN-1
            ...
            scat(1:n,:) = matmul(flux(1:n,:), sigs)
            scat(n+1:init_a,:) = matmul(flux(n+1:init_a,:), sigs_h2o)
      ...
      !Build Source Matrix
      do g=1,gN
         flux(:,g) = matmul(sigma_mat(:,:,g),flux(:,g))
      end do
      
      S=1/k_old*sum(flux, DIM = 2)
